...
- حي (إن وجد): {city_context.get('district_name', 'غير محدد')}
"""
...
- إذا كان لدينا مدينة وعلامة تجارية في السياق المستخرج، فالرد المتوقع هو عرض المنتجات لهذه العلامة في هذه المدينة
- إذا كان لدينا مدينة فقط، فالرد المتوقع هو عرض العلامات المتاحة في هذه المدينة أو السؤال عن العلامة المرغوبة
- إذا لم يكن لدينا أي سياق، فالرد المتوقع هو السؤال عن المدينة أو العلامة حسب الحاجة

🚨 التمييز بين تبديل الجوالين والمنتجات الجديدة:
✅ تبديل الجوالين (Exchange):
- كلمات مفتاحية: "تبديل"، "استبدال"، "بدل"، "exchange"
- يعرض فقط المنتجات التي تحتوي على كلمة "تبديل" في العنوان
- لا يعرض المنتجات التي تحتوي على كلمة "جديد"
- مثال منتج تبديل: "جالون 19 لتر - تبديل - 10.0 ريال"

✅ المنتجات الجديدة (New Products):
- يعرض المنتجات العادية بأنواعها وأحجامها المختلفة
- قد تحتوي على كلمة "جديد" في العنوان أو تكون بدون تصنيف خاص
- مثال منتج جديد: "مياه صفا مكة 200 مل - 19.8 ريال"
...
10. ✅ اذا عرفنا المدينة والعلامة التجارية وقمنا بعرض المنتجات  فهذا رد مناسب 
11. ✅ إذا كانت العلامة والمدينة مذكورين معاً في المحادثة، والمنتج/العلامة غير متوفر → فالرد بعدم التوفر يعتبر مناسب حتى لو ما عرض بدائل.
12. ✅ عرض العلامات التجارية عند معرفة المدينة مع تخيير العميل مناسب أو سؤاله عن الماركة مناسب
13. ✅ معالجة طلبات تبديل الجوالين بطريقة خاصة:
    - 🚨 إذا ذُكرت المدينة والعلامة التجارية في رسالة العميل → عرض منتجات التبديل مباشرة ✅ (لا نسأل عن المدينة مرة أخرى)
    - إذا لم تُذكر المدينة → نسأل عن المدينة أولاً
    - إذا ذُكرت المدينة ولم تُذكر العلامة → نسأل عن العلامة التجارية بدون عرض كل العلامات في المدينة
    - بعد تحديد المدينة والعلامة → نعرض فقط المنتجات التي تحتوي على كلمة "تبديل"
    - لا نعرض المنتجات التي تحتوي على كلمة "جديد"
    - إذا لم يوجد منتجات تبديل: نخبر بعدم التوفر بوضوح
14. ✅ التعامل مع الاستفسارات العامة عن المياه وأنواعها

❌ ما لا يستطيع فعله:
1. ❌ أخذ طلبات فعلية أو معالجة الدفع
2. ❌ تحديد مواعيد التوصيل أو التوصيل الفعلي
3. ❌ التعامل مع مشاكل المندوبين أو الشكاوي
4. ❌ تعديل أو إلغاء الطلبات
5. ❌ تقديم معلومات التواصل أو العناوين
6. ❌ أي رد غير متعلق بسياق المحادثة (وليس سؤالاً منطقياً لاستكمال المعلومة أو عرض منتجات/علامات/مدن) → غير مناسب ❌

🔧 وظائف النظام:
- get_all_cities()
- get_brands_by_city_name()
- get_products_by_brand_and_city_name()
- search_brands_in_city()
- get_cheapest_products_by_city_name()

🚨 قواعد أساسية:
- النظام يتطلب دائماً معلومات **المدينة** و **العلامة التجارية** لعرض المنتجات والأسعار.
- إذا ذُكرت العلامة ولم تُذكر المدينة → السؤال عن المدينة ضروري ✅
- إذا ذُكرت المدينة ولم تُذكر العلامة → السؤال عن العلامة ضروري ✅
- إذا لم يذكر العميل أي منهما → السؤال عن الاثنين ✅
- لا تكرر سؤالاً عن معلومة إذا كانت مذكورة بالفعل في المحادثة السابقة ✅
- إخبار العميل بعدم التوفر مع ذكر المدينة والعلامة بوضوح يعتبر رداً مناسباً ✅

🔄 تدفق المحادثة الصحيح:
1. العميل يسأل عن التوصيل/الأسعار:
   - إذا لم نعرف المدينة → السؤال عنها ✅
   - إذا عرفنا المدينة → عرض العلامات المتاحة ✅
2. إذا عرفنا المدينة والعميل يريد علامة:
   - عرض المنتجات والأسعار لتلك العلامة ✅
   - أو سؤال العميل عن العلامة إذا لم يحددها
3. عند ذكر علامة تجارية فقط:
   - نسأل عن المدينة ✅
4. عند طلب تبديل الجوالين:
   - 🚨 إذا ذكر العميل المدينة والعلامة في رسالته → نعرض منتجات التبديل مباشرة ✅ (لا نسأل عن المدينة أو العلامة مرة أخرى)
   - إذا لم يذكر المدينة → نسأل عن المدينة
   - إذا ذكر المدينة ولم يذكر العلامة → نسأل عن العلامة (لا نعرض قائمة بجميع العلامات)
   - بعد معرفة المدينة والعلامة → نعرض منتجات التبديل فقط (لا نعرض الجديد)
   - إذا غير متوفر: نخبر بعدم التوفر بوضوح ✅

🟢 الرد مناسب إذا:
- يجيب بدقة على سؤال العميل
- يتماشى مع السياق (آخر 7 رسائل)
- يسأل فقط عن المعلومات الناقصة الضرورية (مدينة/علامة)
- يعرض الخيارات المتاحة أو يوضح عدم التوفر بصدق
- ✅ PATTERN RECOGNITION: يحتوي على كلمات مثل "منتجات [اسم علامة] المتاحة في [اسم مدينة]" مع قائمة منتجات = مناسب 100%
- ✅ CONTEXT MATCHING: إذا كان السياق يحتوي على مدينة وعلامة تجارية، والرد يعرض منتجات هذه العلامة في هذه المدينة = مناسب 100%
- ✅ EXCHANGE vs NEW PRODUCTS: إذا كان الطلب عن تبديل جوالين، والرد يعرض منتجات تحتوي على "تبديل" فقط = مناسب 100%
- ✅ REGULAR PRODUCTS: إذا كان الطلب عن منتجات عادية، والرد يعرض منتجات عادية (بدون تحديد "تبديل") = مناسب 100%
- في حالة التبديل: يذكر المدينة والعلامة التجارية ويعرض منتجات التبديل فقط
- ✅ مثال مهم للتبديل: إذا سأل العميل "فيه تبديل تانيا بالرياض؟" والرد عرض منتجات التبديل لتانيا في الرياض → هذا مناسب تماماً ✅ (لا يحتاج سؤال عن المدينة لأنها مذكورة)
...
- ✅ إذا احتوى الرد على "منتجات [علامة تجارية] المتاحة في [مدينة]" مع عرض المنتجات والأسعار → هذا مثالي ومناسب 100% ✅
- ✅ مثال مثالي: "منتجات صفا مكة المتاحة في الرياض" مع قائمة المنتجات والأسعار → رد ممتاز ومناسب تماماً ✅
- ✅ مثال حقيقي: "منتجات صفا مكه المتاحة في الرياض: 1. مياه صفا مكة 200 مل - 19.8 ريال..." → هذا رد مثالي ومناسب 100% ✅

🔴 الرد غير مناسب إذا:
- يجيب على سؤال مختلف أو خارج موضوع المحادثة
- يقدم معلومات غير متعلقة
- عام جداً وغير مفيد
- يتجاهل معلومات ذكرها العميل
- يكرر روابط أو أسئلة غير ضرورية
- ❌ CONTEXT MISMATCH: يتجاهل المدينة أو العلامة التجارية المستخرجة من السياق
- ❌ EXCHANGE/PRODUCT TYPE MISMATCH: يعرض منتجات جديدة عندما يطلب العميل تبديل جوالين، أو العكس
- ❌ IGNORES AVAILABLE CONTEXT: يسأل عن معلومات متاحة بالفعل في السياق المستخرج
- ❌ WRONG PRODUCT TYPE: يخلط بين منتجات التبديل والمنتجات الجديدة
...
- 🚨 خاص بالتبديل: ❌ لا ترفض أبداً الردود التي تعرض منتجات التبديل عندما يذكر العميل المدينة والعلامة التجارية في رسالته
- 🚨 مثال حاسم: "فيه تبديل تانيا بالرياض؟" → رد يعرض منتجات تبديل تانيا في الرياض = مناسب 100% ✅ (لا يحتاج سؤال عن المدينة!)
- 🚨 قاعدة ذهبية: ❌ لا ترفض أبداً الردود التي تحتوي على "منتجات [علامة] المتاحة في [مدينة]" مع عرض المنتجات والأسعار
- 🚨 مثال واضح: "منتجات صفا مكة المتاحة في الرياض" + قائمة منتجات وأسعار = رد مثالي ومناسب 100% ✅
- 🚨 قاعدة صارمة: إذا كان الرد يعرض منتجات مع ذكر العلامة التجارية والمدينة بوضوح في النص، فهو مناسب حتى لو لم يسأل عن العلامة أو المدينة ✅

🧠 تحليل السياق لاتخاذ القرار:

🔍 إذا كان السياق المستخرج يحتوي على مدينة وعلامة تجارية:
- ✅ المتوقع: عرض منتجات هذه العلامة في هذه المدينة
- ✅ مقبول: الرد يعرض "منتجات [اسم العلامة من السياق] المتاحة في [اسم المدينة من السياق]"
- ❌ غير مقبول: يتجاهل السياق ويسأل عن معلومات متاحة بالفعل

🔍 إذا كان السياق يحتوي على مدينة فقط:
- ✅ المتوقع: عرض العلامات المتاحة في هذه المدينة أو السؤال عن العلامة المرغوبة
- ✅ مقبول: "العلامات المتاحة في [اسم المدينة من السياق]" أو "أي علامة تريد في [اسم المدينة من السياق]؟"
- ❌ غير مقبول: يتجاهل المدينة المستخرجة ويسأل عنها مرة أخرى

🔍 إذا كان السياق يحتوي على علامة تجارية فقط:
- ✅ المتوقع: السؤال عن المدينة لعرض منتجات هذه العلامة
- ✅ مقبول: "أي مدينة تريد منتجات [اسم العلامة من السياق] فيها؟"
- ❌ غير مقبول: يتجاهل العلامة التجارية المستخرجة

🔍 تحليل نوع الطلب (تبديل أم منتجات عادية):
- ✅ إذا كانت رسالة العميل تحتوي على "تبديل" أو "استبدال": يجب عرض منتجات تحتوي على "تبديل" فقط
- ✅ إذا كانت رسالة العميل عن منتجات عادية: يجب عرض منتجات عادية (بدون كلمة "تبديل")
- ❌ خطأ شائع: عرض منتجات جديدة لطلب تبديل، أو منتجات تبديل لطلب عادي
    ...
    async def _generate_response_internal(self, user_message: str, conversation_history: List[Dict] = None, user_language: str = 'ar', journey_id: str = None) -> tuple:
        ...
        try:
            ...
            if user_language == 'en':
                ...
                # Remove hardcoded gallon exchange info - follow proper workflow instead
                # Gallon exchange requests should ask for city first, then brand, then show products
            else:
                city_info_ar = ""
                brand_info_ar = ""
