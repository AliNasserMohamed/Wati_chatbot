...
class QueryAgent:
    ...
    def __init__(self):
        ...
        🚨 قاعدة مهمة: اعتبر الأسئلة عن توفر التوصيل الآن أو الحين كـ "not_relevant"
        - مثال: "هل متوفر توصيل الآن؟" → not_relevant
        - مثال: "هل يمكن التوصيل الحين؟" → not_relevant
        - مثال: "متوفر توصيل اليوم؟" → not_relevant"""
        ...
        self.classification_prompt_en = """You are a smart message classifier for a water delivery company. Your task is to determine if a message is related to the company's services or not.
            ...
            🚨 Important rule: Consider questions about delivery availability now/right now as "not_relevant"
            - Example: "Is delivery available now?" → not_relevant
            - Example: "Can you deliver right now?" → not_relevant
            - Example: "Is delivery available today?" → not_relevant"""
...
🚨 تحذير مهم جداً - التفريق بين الماركة والمدينة:
- إذا ذكر العميل "صفا مكة" أو "صفا مكه" فهذا اسم علامة تجارية للمياه وليس مدينة مكة المكرمة
- "صفا مكة" = ماركة مياه (ليس مدينة)
- "مكة المكرمة" أو "مكة" منفردة = مدينة
- لا تستخرج "مكة" كمدينة إذا كانت جزء من "صفا مكة" أو "صفا مكه"
- مثال: "أريد مياه صفا مكة" = لا تستخرج مدينة (هذه علامة تجارية)
- مثال: "أنا في مكة المكرمة" = استخرج مدينة مكة (مرجع جغرافي واضح)

هل استخراج "{extracted_city}" صحيح ومبرر من رسائل العميل فقط؟

أجب بـ "صحيح" إذا كان العميل ذكر المدينة بوضوح كموقع جغرافي، أو "خطأ" إذا لم يذكرها أو كانت جزء من اسم علامة تجارية."""
            ...
            response = await self._call_langchain_llm(
                messages=[
                    {"role": "system", "content": "أنت خبير في فهم النصوص واستخراج المعلومات الجغرافية. كن دقيقاً جداً في التحقق. مهم جداً: فرق بين علامة 'صفا مكة' التجارية ومدينة 'مكة المكرمة' - لا تستخرج مدينة مكة إذا كانت جزء من اسم علامة تجارية."},
...
6. ✅ إذا كان العميل كتب اسم العلامة التجارية منفرداً في رسالة (مثل "نقي" أو "نستله") فهذا ذكر صريح وواضح - صحيح
7. ✅ الرسائل القصيرة التي تحتوي على اسم علامة تجارية فقط تعتبر ذكر صريح للعلامة - صحيح
...
أمثلة صحيحة:
- العميل كتب: "نقي" → صحيح (ذكر صريح للعلامة)
- العميل كتب: "أريد مياه نستله" → صحيح (ذكر واضح)
- العميل كتب: "هاجر" → صحيح (اسم علامة تجارية منفرد)
- العميل كتب: "مياه صفا مكة" → صحيح (ذكر العلامة مع كلمة مياه)

أمثلة خاطئة:
- المساعد: "عندنا نستله وأكوافينا" ، العميل: "نعم" → خطأ (لم يحدد أي علامة)
- المساعد: "أي علامة تريد؟" ، العميل: "أي واحدة" → خطأ (غامض)"""

            print(f"🔍 [BRAND VERIFICATION] Sending prompt to LLM for verification")
            
            # Special handling for exact matches - they should almost always be correct
            system_prompt = "أنت خبير في فهم النصوص واستخراج أسماء العلامات التجارية. كن مرناً مع تنويعات كتابة أسماء العلامات التجارية ولكن دقيقاً في التحقق من ذكر العميل لها."
            
            if "مطابقة تامة" in extraction_source or "exact match" in extraction_source.lower():
                system_prompt += " ملحوظة خاصة: هذا استخراج بمطابقة تامة، لذا كن أكثر مرونة في قبول الذكر الصريح للعلامة."
    ...
    def get_products_by_brand_and_city_name(self, brand_name: str, city_name: str, user_language: str = 'ar') -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if not products:
                    if user_language == 'ar':
                        error_msg = f"عذراً، العلامة التجارية {brand_name} غير متوفرة في  {city_name} حالياً"
                    else:
...
"""
    ...
    async def _validate_response_appropriateness(self, user_message: str, generated_response: str, conversation_history: List[Dict] = None, user_language: str = 'ar', city_context: Dict = None, brand_context: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_language == 'ar':
                                validation_prompt = f"""أنت مقيم مرن لجودة الردود في خدمة العملاء لشركة أبار لتوصيل المياه.

                مهمتك: تحديد ما إذا كان الرد المُولد مناسب ومتعلق برسالة العميل وحادثته أم لا. ولطالما هذه الردود متعلقة بسياق المحادثة وماركات المياه والمدن والمنتجات والاسعار فهي ردود صحيحة ومناسبة.
                ...
                🔄 فهم تدفق المحادثة - مهم جداً لتقييم الردود:
                نحن نقود المحادثات بطريقة منظمة: المدينة أولاً → العلامة التجارية → المنتجات → الرد

                ✅ الردود المناسبة حسب مرحلة المحادثة:
                1. إذا لم نعرف المدينة → السؤال عن المدينة مناسب ✅
                2. إذا عرفنا المدينة لكن لا نعرف العلامة التجارية → السؤال عن العلامة التجارية مناسب ✅  
                3. إذا عرفنا المدينة والعلامة التجارية → عرض المنتجات مناسب ✅
                4. إذا لم نعرف المعلومات → السؤال عن المعلومات الناقصة مناسب ✅
                5. إذا ذكر العميل معلومات جزئية → طلب المعلومات المفقودة مناسب ✅

                🚨 ردود صحيحة يجب قبولها:
                - "أي مدينة أنت فيها؟" عندما لا نعرف المدينة ✅
                - "أي علامة تجارية تريد؟" عندما لا نعرف العلامة التجارية ✅
                - عرض قائمة العلامات التجارية في مدينة معروفة ✅
                - عرض منتجات علامة تجارية في مدينة معروفة ✅
                - إخبار العميل أن المدينة/العلامة التجارية غير متوفرة ✅
                - الردود التي تحتوي على خطوات وسيطة (مثل "دعني أتحقق من العلامات المتاحة في مدينتك") ✅
                - الردود التقدمية التي تبني على الخطوة السابقة ✅
                - طرح سؤال واحد في كل مرة للحصول على معلومة مفقودة ✅

                ⚠️ لا ترفض الردود لمجرد أنها لم تجب على كل شيء في رسالة واحدة - المحادثات التقدمية مقبولة

                🔄 أمثلة عملية لردود مقبولة:
                - عميل: "أريد مياه نستله" → رد: "أي مدينة أنت فيها؟" ✅ (جمع معلومات ناقصة)
                - عميل: "عندكم توصيل الرياض؟" → رد: "نعم، أي علامة تجارية تريد؟" ✅ (تأكيد + سؤال)  
                - عميل: "كم سعر المياه؟" → رد: "أي مدينة ومن أي علامة تجارية؟" ✅ (جمع معلومات)
                - عميل: "أريد مياه" → رد: "أي مدينة أنت فيها؟" ✅ (بداية المحادثة)
                - رد يحتوي على قائمة منتجات أو أسعار ✅ (إنجاز المهمة)
                ...
                2️⃣ إذا عرفنا المدينة والعميل يريد علامة معينة أو يسأل عن الأسعار:
                   ...
                   - أو اذا كانت العلامة غير متوفرة في المدينة وذكرنا ذلك فهو مناسب سواء ذكرنا معه علامات بديلة او لا 
                ...
                3️⃣ عندما يذكر العميل منتج معين :
                   - إذا لم نعرف المدينة → السؤال "أي مدينة أنت فيها؟" مناسب ✅
                  - اذا كان هذا المنتج غير متوفر وقمنا باقتراح علامات تجارية بديلة فهذا رد مناسب جدا 
                
                
                4️⃣ خدمة العملاء المفيدة:
                   - عرض الخيارات المتاحة أفضل من مجرد السؤال
                   اذا كانت العلامة غير متوفرة في المدينة وذكرنا ذلك فهو مناسب سواء ذكرنا معه علامات بديلة او لا
                ...
                - العميل يسأل عن علامة معينة → "هذه العلامة غير متاحة في الرياض أو اي مدينة اخري" ✅ (مناسب)
                ...
                - العميل يطلب تبديل جوالين لعلامة تجارية في مدينة محددة → رد يوضح عدم التوفر مع ذكر العلامة والمدينة بوضوح ✅ (مناسب ومقبول)
                 ❇️ جميع العلامات التجارية التي يعرضها النظام (حتى لو أسماؤها شائعة كمشروبات غازية مثل بيبسي أو كوكا كولا) تعتبر مرتبطة بالمياه ضمن نطاق النظام. 
   ⚠️ لا ترفض الرد لمجرد أن اسم العلامة يبدو غير متعلق بالمياه، ما دام جاء من النظام.
                ...
                - ✅ إذا كان الرد يقول "خدمة تبديل الجوالين للعلامة التجارية [العلامة] غير متوفرة في مدينة [ ثم اسم المدينة ]" مع ذكر العلامة والمدينة بوضوح → هذا مناسب جداً ومقبول ✅
                ...
                - ❌ لا ترفض الردود التي تخبر بصدق عن عدم توفر خدمة التبديل مع ذكر العلامة والمدينة بوضوح ❌
                 مهم جدا جدا جدا جدا -  ويجب التركيز عليه بشكل كبير 
                - البوت يسعي دائما لعرض العلامات البديلة عند عدم توفر منتج او علامة تجارية معينة فهذه ردود مناسبة ومقبولة
                - لا تقم برفض الردود التي تعرض منتجات او علامات بديلة عند طلب العميل لكراتين مياه 
                - لا ترفض اسئلة البوت عن المدينة او العلامة التجارية او المنتج 
                لا ترفض ردود البوت طالما تصح كرد علي المحادثة فالرد بالنسبة لنا افضل من عدم الرض طالما في السياق 
                - اذا قام البوت بعرض المنتجات عند ذكر العميل لعلامة تجارية معينة فذا رد مناسب جدا 
                ملحوظة مهمة جدا
                كل العالامات التجارية التي تاتي في الرد هي علامات تجارية للمياه حتي لو كنت اعتقدت انها لمشروبات اخري فهي علامات للمياه ولكننا لا نكتب كلمة مياه قبلها 
                9- ❇️ إذا ذكر العميل علامة تجارية فقط (مثل "بيبسي" أو "فولفيك") بدون تفاصيل:
   - يمكن للرد أن يعرض مباشرة قائمة المنتجات المتاحة لهذه العلامة في المدينة الافتراضية أو آخر مدينة مذكورة = مناسب ✅
   - أو يمكن للرد أن يسأل سؤالاً استيضاحياً (مثال: "هل ترغب في معرفة المتاح في مدينتك؟") = مناسب ✅
   ⚠️ كلا الخيارين صحيح، ولا يجب رفض الرد بسبب اختيار أحدهما.
                   مهمتك: تحديد ما إذا كان الرد المُولد مناسب ومتعلق برسالة العميل وحادثته أم لا. ولطالما هذه الردود متعلقة بسياق المحادقة وماركات المياه والمدن والمنتجات والاسعار فهي ردود صحيحة ومناسبة 

                ركز في الردود التي ترفضها وراجعها اكثر من مرة قبل الرفض 
                
                🎯 مبادئ التقييم:
                - إذا كان الرد متعلق بالمياه/المدن/العلامات التجارية/الأسعار → اقبله
                - إذا كان يسأل عن معلومات ناقصة لإكمال الطلب → اقبله
                - إذا كان يتبع التدفق المنطقي للمحادثة → اقبله
                - كن مرناً - الهدف مساعدة العميل وليس رفض الردود
                ...
                #        - يمكن للرد أن يكتفي بالتوضيح "غير متوفر" فقط.  
                #        - أو يمكنه إضافة بدائل متاحة في نفس المدينة.  
                #        - كلاهما (مع أو بدون بدائل) = رد مناسب ✅ حتى لو العميل طلب كمية (كراتين/علب). 
                #     7- ❇️ عرض قائمة المنتجات أو قائمة المدن أو قائمة العلامات = رد مناسب دائماً طالما ضمن نطاق المياه  
                #     8- ❇️ عرض العلامات التجارية المتاحة في مدينة مذكورة = رد مناسب دائماً، حتى لو كانت أسماء العلامات تبدو غير مرتبطة بالمياه (مثل بيبسي أو كوكا كولا).  
                #        - التقييم لا يعتمد على الاسم، بل على وجود العلامة في النظام كجزء من منتجات المياه.
                ...
                #     - اجاب برد غير متعلق بسسياق المخادثة او متعلق بموعد توصيل او مندوب 

                #     🟢 الرد مناسب إذا:
                #     - يلتزم بالسياق (مدينة/علامة/نوع المنتج)
                #     - يسأل فقط عن المعلومات الناقصة
                #     - يعرض منتجات أو علامات أو مدن بشكل صحيح لو يبلغ بعدم  التوفر 
                ...
                validation_prompt = f"""You are a flexible quality evaluator for Abar Water Delivery Company customer service responses.

Your task: Determine if the generated response is appropriate and relevant to the customer's message and conversation context. As long as responses relate to water brands, cities, products, and prices, they are correct and appropriate.
...
🔄 Understanding Conversation Flow - Critical for Response Evaluation:
We lead conversations in an organized way: CITY FIRST → BRAND → PRODUCTS → RESPONSE

✅ Appropriate responses by conversation stage:
1. If we don't know the city → Asking about city is appropriate ✅
2. If we know city but not brand → Asking about brand is appropriate ✅  
3. If we know both city and brand → Showing products is appropriate ✅
4. If we don't know information → Asking for missing information is appropriate ✅
5. If customer mentions partial info → Requesting missing info is appropriate ✅

                🚨 Correct responses that MUST be accepted:
- "Which city are you in?" when we don't know the city ✅
- "Which brand do you want?" when we don't know the brand ✅
- Showing list of brands in a known city ✅
- Showing products of a brand in a known city ✅
- Informing customer that city/brand is not available ✅
- Responses with intermediate steps (like "Let me check available brands in your city") ✅
- Progressive responses that build on previous step ✅
- Asking one question at a time to get missing information ✅

                ⚠️ Don't reject responses just because they didn't answer everything in one message - progressive conversations are acceptable

                🔄 Practical examples of acceptable responses:
                - Customer: "I want Nestle water" → Response: "Which city are you in?" ✅ (gathering missing info)
                - Customer: "Do you deliver to Riyadh?" → Response: "Yes, which brand do you want?" ✅ (confirmation + question)  
                - Customer: "What's the price of water?" → Response: "Which city and which brand?" ✅ (gathering info)
                - Customer: "I want water" → Response: "Which city are you in?" ✅ (conversation start)
                - Response containing product list or prices ✅ (task completion)
...
- If response relates to water/cities/brands/prices → Accept it
- If it asks for missing information to complete request → Accept it
- If it follows logical conversation flow → Accept it
- Be flexible - goal is helping customer, not rejecting responses
...
{{"is_appropriate": true/false, "reason": "detailed reason", "confidence": 0.0-1.0}}"""
            ...
            evaluation_result = await language_handler.process_with_openai(
                validation_prompt,
                "أنت خبير تقييم جودة خدمة العملاء. كن مرناً ومساعداً - الهدف قبول الردود المفيدة وليس رفضها." if user_language == 'ar' else "You are a customer service quality evaluation expert. Be flexible and helpful - the goal is accepting useful responses, not rejecting them."
            )
        ...
        try:
            ...
            if user_language == 'en':
                system_message = {
                    ...
                    🚨 CRITICAL BRAND WARNING - EXTREMELY IMPORTANT:
                    - "صفا مكة" or "صفا مكه" is a WATER BRAND NAME, NOT the city of Makkah
                    - When you see "صفا مكة", it's ALWAYS a brand name, NEVER a city reference
                    - DO NOT confuse this brand with the holy city of Makkah
                    - Treat "صفا مكة" exactly like any other water brand (Nestle, Aquafina, etc.)

                    🚨 CRITICAL CITY EXTRACTION WARNING - EXTREMELY IMPORTANT:
                    - When customer says "صفا مكة" or "صفا مكه" they mean the WATER BRAND, not Makkah city
                    - DO NOT assume customer is from Makkah city when they mention "صفا مكة" brand
                    - "صفا مكة" brand ≠ مكة المكرمة city location
                    - Example: "أريد مياه صفا مكة" = customer wants brand, NOT indicating they're in Makkah
                    - Only consider "مكة المكرمة" or standalone "مكة" as city references, never "صفا مكة"
                    ...
                    🚨 CRITICAL QUESTION ORDER - EXTREMELY IMPORTANT:
                    - ALWAYS ask for CITY FIRST, then ask for BRAND
                    - If you know NEITHER city NOR brand: ask for city first, then ask for brand
                    - If you know brand but NOT city: ask for city first, then show products
                    - If you know city but NOT brand: ask for brand, then show products  
                    - If you know BOTH city and brand: directly show products
...
- قوارير هي الجوالين
- دبات مياه = الجوالين 20 لتر = القوارير (نفس المنتج)
...
- مثال: "احتاج كرتونة 48 حبة مقاس 200 مل" = "احتاج كرتونة 48 زجاجة حجم 200 مل"
                    ...
                    🚨 معلومات مهمة عن دبات المياه/الجوالين:
                    - دبات المياه هي الجوالين 20 لتر وهي القوارير
                    - العلامات التجارية المتوفرة لدبات المياه/الجوالين: صافية، تانيا، المنهل
                    - عند سؤال العميل عن الماركات المتاحة لتبديل أو شراء الجوالين، يمكن الاستعانة بهذه البرندات (صافية، تانيا، المنهل)
                    - يجب التأكد أولاً من أن هذه البرندات متاحة في مدينة العميل قبل ذكرها

                    🚨 تحذير مهم جداً للعلامات التجارية - أقرأ بعناية:
                    - "صفا مكة" أو "صفا مكه" هي اسم علامة تجارية للمياه وليست مدينة مكة المكرمة
                    - عندما ترى "صفا مكة" فهي دائماً علامة تجارية، وليس إشارة للمدينة أبداً
                    - لا تخلط هذه العلامة التجارية مع مدينة مكة المكرمة
                    - تعامل مع "صفا مكة" تماماً مثل أي علامة مياه أخرى (نستله، أكوافينا، الخ)

                    🚨 تحذير مهم جداً لاستخراج المدن - اقرأ بعناية شديدة:
                    - عندما يقول العميل "صفا مكة" أو "صفا مكه" فهو يقصد علامة تجارية للمياه وليس مدينة مكة المكرمة
                    - لا تفترض أن العميل من مدينة مكة عندما يذكر علامة "صفا مكة"
                    - "صفا مكة" (العلامة التجارية) ≠ مكة المكرمة (الموقع الجغرافي)
                    - مثال: "أريد مياه صفا مكة" = العميل يطلب العلامة التجارية، وليس يخبرك أنه في مكة
                    - اعتبر "مكة المكرمة" أو "مكة" منفردة كمرجع للمدينة، ولكن أبداً "صفا مكة"
                    ...
                    🚨🔄 أولوية عليا: معالجة تبديل الجوالين - سير عمل خاص وصارم جداً:
                      ...
                      🚨 قاعدة مهمة جداً: تبديل الجوالين يجب أن يكون من نفس الشركة
                      - تبديل جوالين تانيا يجب أن تكون الجوالين الفارغة من تانيا
                      - تبديل جوالين صافية يجب أن تكون الجوالين الفارغة من صافية
                      - تبديل جوالين المنهل يجب أن تكون الجوالين الفارغة من المنهل
                      - لا يمكن تبديل جالون فارغ من شركة بجالون مليء من شركة أخرى
                    ...
                    📍 أسماء المدن المختصرة - قواعد دقيقة جداً:
                    
                    🚨 قاعدة "المدينة" = "المدينة المنورة" - انتبه جداً:
                    ❌ لا تفسر "المدينة" كـ "المدينة المنورة" في هذه الحالات:
                      • أي سؤال يحتوي على: "أي مدينة"، "بأي مدينة"، "في أي مدينة"، "كم مدينة"، "اي مدن"
                      • أسئلة الموقع: "أنت متواجد باي مدينة"، "انت في أي مدينة"، "وين مدينتكم"، "طال عمرك"
                      • أسئلة عامة: "ايش المدن اللي تخدمونها"، "كم مدينة تصلون لها"
                      • عندما يكون "المدينة" جزء من سؤال واضح
                    
                    ✅ فسر "المدينة" كـ "المدينة المنورة" فقط في سياقات تحديد الموقع الواضحة:
                      • إعلان الموقع: "أنا في المدينة"، "أنا من المدينة"، "ساكن في المدينة"
                      • طلب التوصيل: "توصيل المدينة"، "أريد توصيل للمدينة"، "هل تصلون المدينة"
                      • ذكر الموقع مع طلب: "أريد مياه في المدينة"، "عندكم مياه في المدينة"
                      • تأكيد الموقع: "نعم، المدينة" (كرد على سؤال عن المدينة)
                    
                    🚨 قواعد أخرى للمدن المختصرة:
                    - "مكة" أو "مكه" = "مكة المكرمة" (فقط في سياق تحديد الموقع، ليس الأسئلة وليس مع ذكر مياه صفا مكه)
                    - "الخميس" = "خميس مشيط" (فقط في سياق تحديد الموقع، ليس الأسئلة)
                    
                    🔍 كيفية التمييز:
                    - إذا كان العميل يسأل → لا تستخرج مدينة
                    - إذا كان العميل يخبر عن موقعه أو يطلب خدمة لموقع معين → استخرج المدينة
                    - عند الشك، اسأل العميل للتأكيد
                    
                    مثال صحيح: "أنا في المدينة" → المدينة المنورة ✅
                    مثال خاطئ: "أنت متواجد باي مدينة؟" → لا تستخرج مدينة ❌
                    ...
                    - ❌ تجاهل "المدينة" تماماً إذا كانت في سياق سؤال مثل: "أي مدينة"، "بأي مدينة"، "في أي مدينة"، "أنت متواجد باي مدينة"، "طال عمرك"، "وين مدينتكم"
                    ...
                    🚨 اتبع دائماً هذا التسلسل مع الانتباه الشديد لتاريخ المحادثة: المدينة أولاً → العلامة التجارية → المنتجات → الرد

                    🚨 ترتيب الأسئلة المهم جداً - انتبه بقوة:
                    - اسأل دائماً عن المدينة أولاً، ثم اسأل عن العلامة التجارية
                    - إذا كنت لا تعرف المدينة ولا العلامة التجارية: اسأل عن المدينة أولاً، ثم اسأل عن العلامة التجارية
                    - إذا كنت تعرف العلامة التجارية لكن لا تعرف المدينة: اسأل عن المدينة أولاً، ثم اعرض المنتجات
                    - إذا كنت تعرف المدينة لكن لا تعرف العلامة التجارية: اسأل عن العلامة التجارية، ثم اعرض المنتجات
                    - إذا كنت تعرف المدينة والعلامة التجارية: اعرض المنتجات مباشرة
                    ...
                    🚨 أولوية التعامل مع المعلومات الناقصة - المدينة أولاً ثم العلامة التجارية:
                    1. تحقق أولاً من المدينة في رسالة العميل الحالية وفي تاريخ المحادثة
                    2. إذا لم تكن تعرف المدينة - اسأل عنها أولاً: "انت متواجد باي مدينة طال عمرك؟"
                    3. بعد معرفة المدينة، تحقق من العلامة التجارية في الرسالة الحالية وتاريخ المحادثة
                    4. إذا لم تكن تعرف العلامة التجارية - اسأل عنها: "اي ماركة او شركة تريد طال عمرك؟"
                    5. إذا كنت تعرف المدينة والعلامة التجارية - اعرض المنتجات مباشرة
                    
                    🚨 ترتيب مهم: المدينة دائماً أولاً، ثم العلامة التجارية، ثم المنتجات
                    ...
                    - أمثلة: "مياه وي" - "موية نقي" - "مياة أكوافينا" - "مياه نستله" - "مياه صفا مكة"
                    ...
                    - "مياه صفا مكة" سيصبح "صفا مكة" فقط - والذي هو علامة تجارية وليس مدينة
                    ...
                    - "صفا مكة" → هذه علامة تجارية وليس مدينة، إذا لم تعرف المدينة اسأل عنها أولاً
                    ...
                    - إذا سأل عن السعر بدون ذكر العلامة التجارية → اسأل عن المدينة أولاً، ثم "اي ماركة او شركة تريد طال عمرك؟"
                    ...
                    🚨 منع افتراض المدن - مهم جداً وأساسي:
                    - ⚠️ لا يجب أبداً فرض أن العميل من مدينة ما لم يذكر ذلك صراحة
                    - لا تفترض أبداً أي مدينة لأي علامة تجارية أو عميل
                    - إذا ذكر العميل اسم علامة تجارية فقط (مثل "مياه المنهل" أو "نستله") بدون مدينة:
                      ...
                      → لا تفترض "المدينة المنورة" أو أي مدينة أخرى مهما كانت الظروف
                    - استدعي وظائف البحث فقط عندما تملك معلومات صريحة عن المدينة من:
                      → رسالة العميل الحالية
                      → تاريخ المحادثة
                    - 🚨 قاعدة أساسية: عدم ذكر العميل للمدينة = عدم معرفة المدينة = وجوب السؤال عنها
                    ...
                    🚨 قواعد مهمة للتعامل مع الأحياء والمدن:
                    - إذا ذكر العميل حي ثم المدينة: نستخدم المدينة فقط في المقارنة والبحث
                    - إذا ذكر العميل اسم حي فقط: نسأله عن المدينة
                    - مثال: "أنا في حي الملز الرياض" → استخدم "الرياض" فقط للبحث
                    - مثال: "أنا في حي النزهة" → اسأل "أي مدينة؟ النزهة في أي مدينة؟"
                    -  مثال: "أنا في  النزهة" وانت تعتقد ان النهة حي → اسأل "أي مدينة؟ النزهة في أي مدينة؟"
                    🚨 توضيح مهم للأسعار:
